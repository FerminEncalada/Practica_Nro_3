...
@router.route('/grafos')
def mapagrafos():
    r = requests.get("http://localhost:8099/api/Hospital/mapearlosgrafos")
...
def adyacenciasaleatorias():
    r = requests.get("http://localhost:8099/api/Hospital/misgrafos")
...
def nueva_adyacencia():
    ...
    r = requests.get("http://localhost:8099/api/Hospital/adyacencias")
...
def calcular_camino_corto(origen, destino, algoritmo):
    try:
        ...
        url = f"http://localhost:8099/api/Hospital/caminocorto/{origen}/{destino}/{algoritmo}"
        r = requests.get(url)
...
def ejecutar_dfs(origen):
    try:
        ...
        url = f"http://localhost:8099/api/Hospital/dfs/{origen}"
        r = requests.get(url)
